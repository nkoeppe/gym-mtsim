...
class MtSimulator:
    ...
    def margin_level(self) -> float:
        ...
        return float("inf") if margin == 0.0 else self.equity / margin
    ...
    def create_order(
        ...
        if self.hedge:
            return self._create_hedged_order(
                order_type, symbol, volume, fee, fee_type, sl, tp, sl_tp_type
            )
    ...
    def _create_unhedged_order(
        ...
        if old_order.type == order_type:
            new_order = self._create_hedged_order(
                order_type, symbol, volume, fee, fee_type, sl, tp, sl_tp_type
            )
    ...
    def _update_order_profit(self, order: Order) -> None:
        ...
        order.gross_profit = local_gross_profit * self._get_unit_ratio(
            order.symbol, order.exit_time
        )
